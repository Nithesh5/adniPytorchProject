...
class ADNIDataloaderAllData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img_shape != target_img_shape:
            ...
            #print(self.df.iloc[index, 0],",",self.df.iloc[index, 1])
        ...
        #mean = np.mean(resampled_data_arr)
        #std = np.std(resampled_data_arr)
        #resampled_data_arr = (resampled_data_arr - mean) / std
